...
myLR_terameters = MyLR(theta=[[1.0], [1.0]], alpha=2e-4, max_iter=100000)
myLR_terameters.fit_(X3[:, 0].reshape(-1, 1), Y)
y_pred = myLR_terameters.predict_(X3[:, 0].reshape(-1, 1))
print("\n--- Terameters ---")
print("theta   :\n", myLR_terameters.theta)
print("my mse   :\n", myLR_terameters.mse_(y_pred, Y))
plt.grid(True, which="both", linestyle="-", linewidth=0.5)
